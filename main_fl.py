...
def main():
    ...
    # record the metrics (acc & loss) for different trails
    fl_test = [[0.0, 0.0] for _ in range(options['repeat'])]
    model_source_only_test = [[0.0, 0.0] for _ in range(options['repeat'])]
    model_ft_test = [[0.0, 0.0] for _ in range(options['repeat'])]
    model_target_only_test = [[0.0, 0.0] for _ in range(options['repeat'])]
    model_random_test = [[0.0, 0.0] for _ in range(options['repeat'])]
    ...
    for repeat_i in range(options['repeat']):
        ...
        _, _, fl_test[repeat_i][0], fl_test[repeat_i][1] = trainer.train()
        ...
        if not options['noft']:
            ...
            model_target_only_test[repeat_i][0], model_target_only_test[repeat_i][1] = model_target_only_results[2], model_target_only_results[3]
            model_ft_test[repeat_i][0], model_ft_test[repeat_i][1] = model_ft_results[2], model_ft_results[3]
            model_random_test[repeat_i][0], model_random_test[repeat_i][1] = model_random_results[2], model_random_results[3]
            model_source_only_test[repeat_i][0], model_source_only_test[repeat_i][1] = model_source_only_results[2], model_source_only_results[3]

    print('fl_test_mean', np.mean(fl_test, axis=0))
    print('model_source_only_test_mean', np.mean(model_source_only_test, axis=0))
    print('model_ft_test_mean', np.mean(model_ft_test, axis=0))
    print('model_target_only_test_mean', np.mean(model_target_only_test, axis=0))
    print('model_random_test_mean', np.mean(model_random_test, axis=0))
    
    final_metrics = {'fl_test': fl_test,
                     'model_source_only_test': model_source_only_test,
                     'model_ft_test': model_ft_test,
                     'model_target_only_test': model_target_only_test,
                     'model_random_test': model_random_test}
